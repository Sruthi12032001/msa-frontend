...
circles: L.Circle[] = [];
  ...
  reloadData(): void {
    ...
    //     this.shipdata = data as ShipInfo[];
        this.initMap(); // Reset the map to clear all markers and circles
        this.addMarkersAndCircles(); // Add new markers and circles based on new data
    //   },
    //   (error) => {
  ...
  addMarkersAndCircles(): void {
    this.shipdata.forEach((ship) => {
      if (ship.latitude && ship.longitude) {
        ...
        if (ship.severity) {
          const circle = this.createCircle(ship);
  ...
  createMarker(ship: any): L.Marker {
    const icon = this.getIconForZoom(this.map.getZoom());
    return L.marker([ship.latitude, ship.longitude], { icon }).on('click', () => this.dialog.open(DisplayComponent, {
      width: 'auto',
  ...
  createCircle(ship: any): L.Circle {
    const radius = this.baseCircleRadius;
    return L.circle([ship.latitude, ship.longitude], {
      color: 'red',
      fillColor: '#f03',
      fillOpacity: 0.3,
      radius: radius,
